...
//	*pGpioAModereg |= 0x00000400;
	*pGpioAModereg |= (1 << 10);
	...
	//clear bit 0 and 1 in gpioa mode register
	*pGpioAModereg &= ~(3 << 2);

	while(1) { //led will toggle infinitely

		uint8_t pin_read = (uint8_t)(*pGpioInReg & 0x2);
		...
		//	*pGpioAOutreg |= 0x0020;
			*pGpioAOutreg |= (1 << 5);
			...
			/*clear the 5th bit in gpioa output data register to turn off led*/
			*pGpioAOutreg &= ~(1 << 5);
		}
	}
}
